...
public class ProductMangementServiceImpl implements ProductManagementService{
    ...
    public void registerProduct(ProductManagementRequest.OfRegister registerRequest, String imageUrl) throws RuntimeException {
        try {
            ...
            if(registerRequest instanceof ProductManagementRequest.OfOfRegisterEcoFriendly) {
                product.setIsCertification(ActiveStatus.ACTIVE);

                ProductManagementRequest.Certification certification = (
                        (ProductManagementRequest.OfOfRegisterEcoFriendly) registerRequest).getCertification();
    ...
    @Override
    @Transactional
    public boolean modifyProduct(Long productId,  Long customerId, ProductManagementRequest.OfModifying ofModifying) {
        Product product = productRepository.findById(productId).orElseThrow(
                () -> new IllegalArgumentException("해당하는 상품이 존재하지 않습니다."));
        ...
        product.changeProductName(ofModifying.getProductName());
        product.changeProductDescription(ofModifying.getDescription());
        product.changeProductOrigin(ofModifying.getOrigin());
        product.changeProductPrice(ofModifying.getPrice());
        product.changeBaseDiscountRate(ofModifying.getBaseDiscountRate());
        product.changePersonalizedDiscountRate(ofModifying.getPersonalizedDiscountRate());
        product.changeRegularDiscountRate(ofModifying.getRegularDiscountRate());
        product.changeIsPageVisibility(ActiveStatus.fromCode(ofModifying.getIsPageVisibility()));
        product.changeIsRegularSale(ActiveStatus.fromCode(ofModifying.getIsRegularSale()));
        return true;
    }

    /**
     * 고객이 등록한 상품리스트 조회 -> 이거 친환경 여부를 넣어야한다.
     * @param customerId
     * @return
     */
    @Override
    ...
    @Override
    public ProductManagementResponse.OfRetrieveProductOrder retrieveProductInformatino(Long productId) {
        Product product = productRepository.findById(productId).orElseThrow(
    ...
    @Transactional
    public void registerCertificationOf(Product product, ProductManagementRequest.OfRegister registerRequest) {
        ProductManagementRequest.Certification certification
                = ((ProductManagementRequest.OfOfRegisterEcoFriendly) registerRequest).getCertification();
